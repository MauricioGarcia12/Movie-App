...
<div class="relative mt-3 md:mt-0" x-data="{ isOpen: true }" @click.away="isOpen = false">
    <!--
        click.away: When clicking away the bar shows off
        keydown: When pressing esc the bar shows off
        focus:when focusing on the searchbar the dropdown shows it again
        keydown.shift.tab: When pressing those keys the dropdown hide
        keydown="isOpen =true ": makes for default the variable true
    ...
    @if (strlen($search) >= 2)
    <!--
        z-50: To the image behind the dropdown
        x-show.transition.opacity: will do an opacity 
     -->
        <div
                ...
                <!--Using the variables from the liveware file-->
                    @foreach ($searchResults as $result)
                        <li class="border-b border-gray-700">
                            <!--If we do tab on each movie on the dropdown after the final one the dropdown will hide-->
                            <a 
